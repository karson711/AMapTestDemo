...
#import "ZJCommenDefine.h"
#import "CALayer+ZJUI.h"

@interface ZJCustomButton ()
...
- (void)initialization
{
    ...
    self.buttonImagePosition = ZJCustomButtonImagePositionLeft;//默认布局
}
...
- (void)layoutSubviews
{
    ...
    //默认布局不用处理
    if (self.buttonImagePosition == ZJCustomButtonImagePositionLeft) {
        return;
    }
    //content的实际size
    ...
    //垂直布局
    if (self.buttonImagePosition == ZJCustomButtonImagePositionTop || self.buttonImagePosition == ZJCustomButtonImagePositionBottom) {
        ...
        if (self.buttonImagePosition == ZJCustomButtonImagePositionTop) {//重置Y坐标
    ...
    } else if (self.buttonImagePosition == ZJCustomButtonImagePositionRight) {//水平布局
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    switch (self.buttonImagePosition) {
        case ZJCustomButtonImagePositionTop:
        case ZJCustomButtonImagePositionBottom: {
        ...
        case ZJCustomButtonImagePositionLeft:
        case ZJCustomButtonImagePositionRight: {
            if (self.buttonImagePosition == ZJCustomButtonImagePositionLeft && self.titleLabel.numberOfLines == 1) {
...
- (void)setButtonImagePosition:(ZJCustomButtonImagePosition)buttonImagePosition
{
...
- (void)adjustsButtonHighlighted {
    if (self.highlightedBackgroundColor) {
        if (!self.highlightedBackgroundLayer) {
            ...
            [self.highlightedBackgroundLayer ZJ_removeDefaultAnimations];//移除系统默认的隐性动画
